...
export async function unzipWordPress(
	...
	// The core minified bundle ships as a solid `tar.zst`; every other source
	// (CLI wordpress.org releases, custom URLs, nightly/trunk GitHub master.zip,
	// arbitrary API-supplied ZIPs, GitHub artifacts) is a ZIP. Sniff the first
	// bytes and route: stream-extract tar.zst into MEMFS, or fall back to the
	// PHP `ZipArchive` path. The full uncompressed tar is never materialized.
